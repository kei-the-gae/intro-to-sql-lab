...
-- Write SQL query here
SELECT countrycode, language FROM countrylanguages WHERE countrycode = 'VAT' AND isofficial = TRUE;

-- Clue #3: We have new news on the classes Carmen attended – our gumshoes tell us she's moved on to a different country, a country where people speak only the language she was learning. Find out which nearby country speaks nothing but that language.

-- Write SQL query here
SELECT * FROM countrylanguages WHERE language = 'Italian' AND percentage = 100;
SELECT code, name, region FROM countries WHERE code = 'SMR';
...
-- Write SQL query here
SELECT * FROM cities WHERE name LIKE 'San M_____' AND countrycode in ('ARG', 'BOL', 'BRA', 'CHL', 'COL', 'ECU', 'FLK', 'GUF', 'GUY', 'PER', 'PRY', 'SUR', 'URY', 'VEN');
...
-- Write SQL query here
SELECT code, name, capital FROM countries WHERE code = 'ARG';
